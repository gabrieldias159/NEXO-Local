...
if ($doente) {
  ...
  if ($fs.ok -and $FIREBASE) {
    try {
      ...
      else { Log "export pre-reciclagem ok (exit $($p.ExitCode))"; Set-Content $SNAPSTAMP (Get-Date -Format o) -Encoding ASCII }
  ...
  if ($fazer -and $FIREBASE) {
    try {
      ...
      if ($p.WaitForExit(600000) -and $p.ExitCode -eq 0) {
        ...
        try { $p.Kill() } catch {}
        Log "snapshot periodico falhou/estourou - tento no proximo ciclo"
      }
