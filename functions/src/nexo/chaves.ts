/**
 * CHAVES CANÔNICAS do NEXO — lado FUNCTIONS.
 *
 * ESPELHO VERBATIM de `src/lib/nexo/chaves.ts`. Os dois lados PRECISAM produzir
 * exatamente a mesma string para a mesma entrada, senão o JOIN entre o que o
 * coletor (functions) grava e o que o detector (app) procura silenciosamente
 * não casa nada. Por isso este arquivo é uma cópia idêntica em algoritmo do
 * lado do app — só muda a import de `hashDoc` (regra do projeto: `functions/`
 * NÃO importa de `src/`, então cada lado importa o seu próprio `pii.ts`).
 *
 ...
 *
 * Self-contained (igual ao resto de `functions/nexo`): só importa `hashDoc` do
 * `pii.ts` local. Determinístico, sem efeito colateral.
 */
