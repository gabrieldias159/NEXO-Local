 ...
 * materializado), mais constantes de versão/TTL compartilhadas pelo worker, o
 * nightly e a varredura de TTL.
 *
 * ESPELHO: este arquivo é cópia VERBATIM de `src/lib/nexo/jobs-tipos.ts`
 * (regra `functions/` NÃO importa de `src/`). Qualquer mudança aqui deve ser
 * replicada no espelho do app — eles formam o contrato cliente↔worker.
 */
